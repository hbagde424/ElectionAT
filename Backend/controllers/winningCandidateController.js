...
exports.getWinningCandidate = async (req, res, next) => {
  try {
    ...
    if (req.userHierarchy && !(req.user && req.user.role === 'superAdmin')) {
      ...
      const boothId = winningCandidate.booth_id ? String(winningCandidate.booth_id) : null;
      const blockId = winningCandidate.block_id ? String(winningCandidate.block_id) : null;
      const assemblyId = winningCandidate.assembly_id ? String(winningCandidate.assembly_id) : null;
      const parliamentId = winningCandidate.parliament_id ? String(winningCandidate.parliament_id) : null;
      const divisionId = winningCandidate.division_id ? String(winningCandidate.division_id) : null;
      const stateId = winningCandidate.state_id ? String(winningCandidate.state_id) : null;
