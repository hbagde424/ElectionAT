...
function HierarchicalMap({ onRegionClick }) {
    ...
    // Helper to get predicted party for an assembly
    const getPredictedParty = (acNo, name) => {
        // Try to match by acNo or name
        return predictions.find(
            p => (p.assembly_no === acNo || p.assembly_name === name)
        );
    };
            ...
            if (assemblies && assemblies.data[0].type === "FeatureCollection" && assemblies.data[0].features && assemblies.data[0].features.length > 0) {
                const transformedData = {
                    ...
                    features: assemblies.data[0].features.map(feature => {
                        const acNo = feature.properties.AC_NO?.toString();
                        const name = feature.properties.AC_NAME;
                        const pred = getPredictedParty(acNo, name);
                        return {
                            type: 'Feature',
                            properties: {
                                ...
                                category: 'GEN',
                                lastElectionYear: '2023',
                                predicted_party: pred?.predicted_party || null,
                                predicted_win_count: pred?.win_count || 0
                            },
                            geometry: feature.geometry
                        };
                    })
        ...
        switch (level) {
            ...
            case 'assembly':
                content += `
                    ...
                    <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid #eee;">
                        <p><strong>Predicted Winner (2028):</strong></p>
                        ${properties.predicted_party ? `
                            <span style="display:inline-block;font-weight:bold;color:${properties.predicted_party.color || '#333'}">
                                ${properties.predicted_party.name || ''}
                                ${properties.predicted_party.symbol ? `<img src='${properties.predicted_party.symbol}' alt='symbol' style='height:18px;vertical-align:middle;margin-left:4px;'/>` : ''}
                            </span>
                            <span style="font-size:0.9em;color:#888;"> (Past wins: ${properties.predicted_win_count})</span>
                        ` : '<span style="color:#888">No prediction</span>'}
                    </div>
                    <hr style="margin: 10px 0">
