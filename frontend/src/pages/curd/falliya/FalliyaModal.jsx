...
export default function FalliyaModal({
    open,
    modalToggler,
    falliya,
    states = [],
    ...
    useEffect(() => {
        if (falliya) {
            setFormData({
                state_id: falliya.state_id?._id || '',
                division_id: falliya.division_id?._id || '',
                parliament_id: falliya.parliament_id?._id || '',
                assembly_id: falliya.assembly_id?._id || '',
                block_id: falliya.block_id?._id || '',
                booth_id: falliya.booth_id?._id || '',
                panchayat_id: falliya.panchayat_id?._id || '',
                village_id: falliya.village_id?._id || '',
                falliya_name: falliya.falliya_name || '',
                location: falliya.location || '',
                latitude: falliya.latitude || '',
                longitude: falliya.longitude || '',
                male_count: falliya.male_count || '',
                female_count: falliya.female_count || '',
                others_count: falliya.others_count || ''
            });
        } else {
            setFormData({
                state_id: '',
    ...
    useEffect(() => {
        if (formData.panchayat_id) {
            setFilteredVillages(villages.filter(v => getId(v.panchayat_id) === formData.panchayat_id));
        } else {
            setFilteredVillages([]);
        }
    ...
    const validateForm = () => {
        ...
        if (!formData.falliya_name.trim()) newErrors.falliya_name = 'Falliya name is required';

        // Validate falliya name length
        if (formData.falliya_name.length > 100) {
            newErrors.falliya_name = 'Falliya name cannot exceed 100 characters';
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (falliya) {
                await axiosServices.put(`/falliyas/${falliya._id}`, submitData);
            } else {
                await axiosServices.post('/falliyas', submitData);
            }

            modalToggler();
            refresh();
        } catch (error) {
            console.error('Error saving falliya:', error);
        ...
        <Dialog open={open} onClose={handleClose} maxWidth="lg" fullWidth>
            <DialogTitle>
                <Typography variant="h4">
                    {falliya ? 'Edit Falliya' : 'Add New Falliya'}
                </Typography>
            </DialogTitle>
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                        <FormTextField
                            label="Falliya Name"
                            name="falliya_name"
                            value={formData.falliya_name}
                            onChange={handleInputChange}
                            error={errors.falliya_name}
                            required
                        />
                    </Grid>
                ...
                >
                    {isSubmitting ? (
                        <>
                            ...
                            {falliya ? 'Updating...' : 'Creating...'}
                        </>
                    ) : (
                        falliya ? 'Update Falliya' : 'Create Falliya'
                    )}
                </Button>
            </DialogActions>
        </Dialog>
