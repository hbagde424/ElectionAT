    ...
    const loadBoothPolygons = async (currentFilters = appliedFilters) => {
        try {
            ...
            if (blasJson.success && Array.isArray(blasJson.data)) {
                blasJson.data.forEach(bla => {
                    if (bla.booth_id) {
                        boothsWithBLA.add(String(bla.booth_id._id || bla.booth_id));
                    }
                });
            }
            console.log('📊 Booths with BLA data:', boothsWithBLA.size);

            // Extract polygons from booths that have polygon data AND have BLA data
            const features = [];
            boothsToUse.forEach(booth => {
                // Only include booths that have BLA data
                if (booth.polygon && boothsWithBLA.has(String(booth._id))) {
            ...
            if (!features.length) {
                setMapError('No booth polygons found with BLA data for your access level');
            ...
            // Fetch booths with BLA for marker colors
    ...
    return (
        <MainCard content={false}>
            <Stack spacing={2} sx={{ p: 2 }}>
                ...
                <Box sx={{ p: 2, pb: 0 }}>
                    <Typography variant="h6" sx={{ mb: 1 }}>BLA Coverage Map (Showing booths accessible to you)</Typography>
                    {mapError && (
