...
export default function AlertDistrictDelete({ id, open, handleClose, refresh }) {
    const handleDelete = async () => {
        const token = localStorage.getItem('serviceToken');
        const res = await fetch(`${import.meta.env.VITE_APP_API_URL}/districts/${id}`, {
            method: 'DELETE',
            headers: {
        ...
        <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
            <DialogTitle>
                <Stack direction="row" alignItems="center" spacing={1}>
                    ...
                    <Typography variant="h6">Delete District</Typography>
                </Stack>
            </DialogTitle>
            <DialogContent>
                <Stack spacing={2}>
