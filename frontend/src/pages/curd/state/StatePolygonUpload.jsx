...
export default function StatePolygonUpload({ open, onClose, onSuccess }) {
    ...
    const validateGeoJSON = (data) => {
        ...
        data.features.forEach((feature, index) => {
            ...
            if (!feature.geometry.type || !['Polygon', 'MultiPolygon'].includes(feature.geometry.type)) {
                throw new Error(`Feature ${index}: geometry type must be "Polygon" or "MultiPolygon"`);
            }

            if (!Array.isArray(feature.geometry.coordinates)) {
                throw new Error(`Feature ${index}: coordinates must be an array`);
            }
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${import.meta.env.VITE_APP_API_URL}/states/upload-polygon`,
                {
                    method: 'POST',
                    headers,
            ...
            setSuccess(result.message || `✓ State polygon "${geoJsonData.name || 'File'}" uploaded successfully!`);
    ...
    return (
        <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
            <DialogTitle>Upload State Polygon (GeoJSON)</DialogTitle>
            <DialogContent>
