...
export default function InfluancerDetailPage() {
    ...
    const fetchDetails = async () => {
        try {
            ...
            const res = await axiosServices.get(`/influencers/${id}`, { headers: { Authorization: `Bearer ${token}` } });
            if (res.data?.success) setItem(res.data.data);
            else setError('Failed to fetch influencer details');
        } catch (err) {
            console.error(err);
            setError('Error loading influencer details.');
    ...
    if (loading) return (
        <Container maxWidth="lg" sx={{ mt: 2 }}>
            <LinearProgress />
            <Box sx={{ mt: 2 }}><Typography>Loading influencer details...</Typography></Box>
        </Container>
    );
        ...
        <Container maxWidth="lg" sx={{ mt: 2, mb: 4 }}>
            <Box sx={{ mb: 3 }}>
                <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 2 }}>
                    ...
                    <Typography variant="h4" component="h1" sx={{ flexGrow: 1 }}>Influencer Details</Typography>
