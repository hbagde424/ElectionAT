...
import AssemblyVotesModal from 'pages/curd/votes/AssemblyVotes/AssemblyVotesModal';
import AlertAssemblyVotesDelete from 'pages/curd/votes/AssemblyVotes/AlertAssemblyVotesDelete';
import AssemblyVotesView from 'pages/curd/votes/AssemblyVotes/AssemblyVotesView';
...
export default function AssemblyVotesListPage() {
  ...
  const fetchVotes = async (pageIndex, pageSize, filterParams = filters) => {
    ...
    try {
      ...
      console.error('Failed to fetch assembly votes:', error);
    } finally {
      setLoading(false);
    }
  };
  const fetchAllVotesForCsv = async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_APP_API_URL}/assembly-votes?all=true`, { headers });
      const json = await res.json();
      if (json.success) {
        return json.data;
      }
                    ...
                    {row.getIsExpanded() && (
                      <TableRow>
                        <TableCell colSpan={row.getVisibleCells().length}>
                          <AssemblyVotesView data={row.original} />
                        </TableCell>
                      </TableRow>
                    )}
                  </Fragment>
