...
export default function BlockVotesListPage() {
  ...
  const fetchVotes = async (pageIndex, pageSize, currentFilters = filters) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_APP_API_URL}/block-votes?page=${pageIndex + 1}&limit=${pageSize}${queryString}`, { headers });
  ...
  const fetchAllVotesForCsv = async () => {
    ...
    try {
      const token = localStorage.getItem('serviceToken');
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const res = await fetch(`${import.meta.env.VITE_APP_API_URL}/block-votes?all=true`, { headers });
      const json = await res.json();
      if (json.success) {
        return json.data;
      }
        ...
        >
          <TextField
            ...
            onChange={(e) => {
              ...
              searchDebounceRef.current = setTimeout(() => table.setGlobalFilter(String(v)), 500);
            }}
