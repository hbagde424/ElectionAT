    ...
    <MainCard content={false}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ p: 3 }}>
        <DebouncedInput
          ...
          placeholder={`Search ${data.length} districts...`}
        />
      </Stack>
...
export default function DistrictListPage() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/districts?populate=state_id,division_id,assembly_id,parliament_id')
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => <Typography>{row.original.parliament_id?.name || 'N/A'}</Typography>
      },
      {
        header: 'Created At',
  ...
  return <ReactTable data={districts} columns={columns} />;
}