    ...
    <MainCard content={false}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ p: 3 }}>
        <DebouncedInput
          ...
          placeholder={`Search ${data.length} parliaments...`}
        />
      </Stack>
...
export default function ParliamentListPage() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/parliaments?populate=state_id,division_id,assembly_id')
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
          setParliaments(data.data || []);
        }
        setLoading(false);
      })
  ...
  return <ReactTable data={parliaments} columns={columns} />;
}