    ...
    <MainCard content={false}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ p: 3 }}>
        <DebouncedInput
          ...
          placeholder={`Search ${data.length} blocks...`}
        />
      </Stack>
...
export default function BlockListPage() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/blocks?populate=state_id,division_id,district_id,parliament_id,assembly_id')
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
          setBlocks(data.data || []);
  ...
  return <ReactTable data={blocks} columns={columns} />;
}