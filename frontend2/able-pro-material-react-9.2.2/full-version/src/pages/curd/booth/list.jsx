    ...
    <MainCard content={false}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ p: 3 }}>
        <DebouncedInput
          ...
          placeholder={`Search ${data.length} booths...`}
        />
      </Stack>
...
export default function BoothListPage() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/booths?populate=state_id,division_id,district_id,parliament_id,assembly_id,block_id')
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
          setBooths(data.data || []);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => <Typography>{row.original.block_id?.name}</Typography>
      },
      {
        header: 'Assembly',
        accessorKey: 'assembly_id.name',
        cell: ({ row }) => <Typography>{row.original.assembly_id?.name}</Typography>
      },
      {
        header: 'Parliament',
  ...
  return <ReactTable data={booths} columns={columns} />;
}